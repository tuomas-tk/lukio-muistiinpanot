# Kemiakertaus


# Kemia 1
## Aineet
- puhtaat aineet
  - alkuaineet (happi, kulta)
    - metallit (kulta, kalsium)
    - puolimetallit (boori, pii)
    - epämetallit (hiili, xenon)
  - yhdisteet
    - molekyylit (vesi, sokeri)
    - ioniyhdisteet (ruokasuola, salmiakki)
- seokset
  - homogeeniset seokset (suolaliuos, ilma, pronssi)
  - heterogeeniset seokset
    - dispersiot (eri osia ei voi erottaa paljaalla silmällä) (maito, savu)


## Alkuaineiden jaksollinen järjestelmä
- Muodostuu kun alkuaineet laitetaan **järjestysluvun** (protonien (tai elektronien) määrän) mukaiseen järjestykseen.
- Havaitaan, että alkuaineiden ominaisuudet muuttuvat **jaksollisesti**, joten katkaistaan alkuainelista sopivista kohdista niin, että samalla tavalla käyttytyvät alkuaineet tulevat allekain.
- Vaakarivejä kutsutaan **jaksoiksi** ja pystysarakkeita **ryhmiksi**. Ryhmät 1,2 ja 13-18 ovat **pääryhmiä**, välillä 3-12 jäävät **siirtymämetallit**

### Ryhmän jäsenillä on yhtä monta ulkoelektronia
- koska kemialliset reaktiot perustuvat ulkoelektronien pyörittelyyn ympäriinsä, ryhmän jäsenet myös reagoivat samalla tavalla
#### 1. Alkalimetallit
- vain yksi ulkoelektoni, josta yrittävät mielellään päästä eroon (eli ovat kaikista vähiten elektronegatiivisia) -> reagoivat erittäin kiivaasti -> luonnossa vain yhdisteinä
#### 2. Maa-Alkalimetallit
- kaksi ulkoelektronia, eivät reagoi aivan yhtä kiivaasti kuin Alkalimetallit
#### 13. Booriryhmä
- alumiini tärkeä
#### 14. Hiiliryhmä
- hiiliryhmän alkuaineet kaikista erilaisimpia: sekä metalleja että epä- ja puolimetalleja
- hiili tärkeä elämälle
#### 15. Typpiryhmä
- typpi (muodostaa nitraatteja) ja fosfori (muodostaa fosfaatteja) tärkeitä elämälle
#### 16. Happiryhmä
- ulkokuori kahta elektronia vaille oktetissa
- happi (muodostaa oksideja) ja rikki (muodostaa sulfideja) tärkeitä elämälle
#### 17. Halogeenit
- ulkokuori yhtä elektronia vaille oktetissa -> reagoivat helposti metallien kanssa
- vapaina alkuaineina muodostavat kaksiatomisia molekyylejä
#### 18. Jalokaasut
- oktetti -> ei mitään syytä reagoida: hyvin inerttejä
- vapaina alkuaineita yksiatomisia kaasuja

## Alkuaineiden ominaisuudet (jaksollisessa järjestelmässä ja muutenkin)
**TODO** kaavioita tähän

## Vahvat sidokset
Yksinkertaisesti sanottuna:
### 1) Kemialliset sidokset syntyvät ulkoelektronien avulla (siirtymämetalleilla poikkeuksia)
### 2) Metallit reagoivat **luovuttamalla** viimeisiäkin ulkoelektonejaan pois
### 3) Epämetallit **ottavat vastaan** lisää elektroneja täydentämään lähes täyttä elektronikuortaan
Seuraa erilaisia sidoksia:
### Ionisidos (metalli - epämetalli)
- **metalli luovuttaa elektroneja** (hapettuu, hapetusluku positiivinen, muodostaa positiivisen ionin),
- **epämetalli vastaanottaa elektroneja** (pelkistyy, hapetusluku negatiivinen, muodostaa negatiivisen ionin)
- tämä on vain yksinkertaistus: moniatomiset ionit eivät ole metalleja tai epämetalleja
- **molemmat saavat oktetin**
- atomeilla erimerkkiset sähkövaraukset: **vetävät toisiaan puoleensa**
- atomit asettuvat mahdollisimman lähelle toisiaan, kiinteissä aineissa eivät liiku liian nopeasti jolloin jäävät kidemuotoon, muodostaen **ionihilan**
- syntyvää ioniyhdistettä kutsutaan **suolaksi**
### Metallisidos (metalli - metalli)
- alkuaineena metallit muodostavat metallisidoksen, jossa ne **ovat luovuttaneet ulkoelektroninsa pois** *"elektronien mereksi"* atomiydinten ympärille
- elektronit pitävät aineen kasassa
- vapaat elektronit selittävät sähkönjohtavuuden
### Kovalenttinen sidos (epämetalli - epämetalli)
- epämetallit pyrkivät vastaanottamaan elektroneja, joten **muodostavat keskenään sidoksen ottamalla ulkoelektroneja yhteiseen käyttöön**
- voi olla yksin-, kaksin- tai kolminkertainen
- yhteen kovalenttiseen sidokseen osallistuu parittomat elektronit molemmilta atomeilta: molemmat atomit saavat elektronilleen parin
- jos kovalenttisen sidoksen atomien elektronegatiivisuudet ovat erisuuret, yhteiset elektronit ovat enemmän elektronegatiivisemman atomin puolella
  - molekyyliin syntyy **osittaisvarauksia**
  - sidoksesta, ja siten usein myös molekyylistä, tulee **poolinen**, eli se sisältää **pysyviä dipoleja**
- atomin vapaat elektroniparit ovat saaneet parin ilman sidoksia: eivät siis enää reagoi
- jotkin atomit muodostavat **kovalenttisen hilan** jossa on hyvin paljon atomeita sitoutuneena toisiinsa samanlaisilla kovalenttisilla sidoksilla
  - atomit muodostavat säännöllisen "verkoston"
  - esim. hiilen eri *allotroopeilla* (timantti, grafiitti) on kovalenttinen hila
#### Kovalenttisen- ja ionisidoksen raja on häilyvä TODO lisää tästä

## Heikot sidokset
**Lyhyesti: vastakkaismerkkiset (osittais)varaukset vetävät toisiaan puoleensa**

**Seuraa erilaisia sidostyyppejä:**
### Dispersiovoimat
- elektronit liikkuvat atomin ympärillä, ja aina välillä atomin toisella puolella on enemmän elektroneja kuin toisella
- molekyyleihin syntyy **hetkellisiä dipoleja**, jotka ovat heikkoja ja lyhytkestoisia
- nämä varauserot vetävät toisiaan puoleensa, muodostaen atomien/molekyylien välisiä dispersiovoimia (Van der Waalsin voimia)
### Dipoli-dipolisidos
- kahden poolisen molekyylin osittaisvarausten (eli dipolien) välille syntyy vetovoimia
#### Erityistapaus: vetysidos
- dipoli-dipolisidos, jossa dipolin toisessa päässä on vety ja toisessa päässä jokin elektronegatiivinen atomi (fluori, happi, typpi)
- vahvin mahdollinen dipoli-dipolisidos, koska vedyn ja "ahneiden paskiaisten" välinen elektronegatiivisuusero on hyvin suuri
- erityisesti vesimolekyylien välillä vahvoja vetysidoksia, mistä seuraa suuri pintajännitys
### Ioni-dipolisidos
- kuin dipoli-dipolisidos, mutta sidoksen toisena osapuolena onkin joko negatiivisesti tai positiivisesti varautunut ioni




# KE2 Orgaaninen kemia



## Orgaaninen = sisältää hiiltä
- Hiili on yksi harvoista alkuaineista, jotka pystyvät muodostamaan pitkiä ketjuja kovalenttisten sidoksien avulla
- Monimutkaiset hiilivedyt ovat käytännössä elämän edellytys: lähes kaikki eliöiden molekyylit sisältävät hiiltä. Tästä nimi *orgaaninen* kemia.
- Myös esimerkiksi muovit ovat orgaanisia polymeerejä, vaikka eivät esiinny luonnossa (paitsi nykyään esiintyy jo liikaakin). Nimi *orgaaninen* ei siis tarkoita samaa kuin *luonnonmukainen* tai *biohajoava*.

## Yhdisteiden jaottelu ja funktionaaliset ryhmät
- Erilaisia "erikoisuuksia" hiiliketjussa
- Nimensä mukaisesti toimivat: orgaanisten yhdisteiden reaktiot perustuvat funktionaalisiin ryhmiin
- samoja funktionaalisia ryhmiä sisältävät yhdisteet reagoivat samalla tavalla: kun tiedät miten jokin funktionaalinen ryhmä reagoi, tiedät miten kaikki sitä sisältävät molekyylit reagoivat!
- opettele ryhmiä Quizletin avulla: [https://quizlet.com/189477351/learn](https://quizlet.com/189477351/learn)

### Alkaanit (ei funktionaalisia ryhmiä)
- sisältävät ainoastaan yksinkertaisia hiili-hiili sidoksia
  - orbitaalitasolla syntyy 4 kpl sp³-hybridiorbitaaleja, joiden välille muodostuu sigmasidos
- kutsutaan tyydyttyneiksi hiilivedyiksi
  - *muistisääntö: hiilivety on tyydyttynyt kun se on päässyt eroon kaikista kaksois- ja kolmoissidoksista*
- alkeeneja ja alkyynejä kutsutaan tyydyttymättömiksi hiilivedyiksi

### Alkeenit / kaksoissidos
- sisältävät vähintään yhden hiili-hiili **kaksoissidoksen**
  - orbitaalitasolla syntyy 3 kpl sp²-hybridiorbitaaleja, hybridisoitumatta jää yksi 2p-orbitaali
  - sigmasidos sp²-hybridiorbitaalien välille, piisidos 2p-orbitaalien välille
- kaksoissidoksessa kahdella hiiliatomilla on yhteensä 4 yhteistä ulkoelektronia

### Alkyynit / kolmoissidos
- sisältävät vähintään yhden hiili-hiili **kolmoissidoksen**
  - orbitaalitasolla syntyy 2 kpl sp-hybridiorbitaaleja, hybridisoitumatta jää kaksi 2p-orbitaalia
  - sigmasidos sp-hybridiorbitaalien välille, kaksi piisidosta 2p-orbitaalien välille
- kolmoissidoksessa kahdella hiiliatomilla on yhteensä 6 yhteistä ulkoelektronia

### Aromaattiset / bentseenirengas
- sisältävät bentseenirenkaan
- bentseenirenkaassa kaksoissidokset *konjugoituneet* ja elektronit *delokalisoituneet* eli ovat jakautuneet tasaisesti koko renkaan alueelle

![](https://upload.wikimedia.org/wikipedia/commons/e/ec/Benzene-orbitals3.png)

*kuva: Wikimedia Commons / Joonasl*

### Alkoholit / hydroksyyliryhmä (-OH)
- a.k.a. viinat, kuitenkin muutakin kuin vain etanoli
- voi hapettua aldehydiksi
- ei voi pelkistyä
- jaetaan yksi- tai moniarvoisiin alkoholeihin hydroksyyliryhmien määrän mukaan
- jaetaan myös:
  - primäärisiin: hydroksyyliryhmän sisältämässä hiilessä kiinni vain yksi hiili
  - sekundäärisiin: hydroksyyliryhmän sisältämässä hiilessä kiinni kaksi hiiltä
  - tertiäärisiin: hydroksyyliryhmän sisältämässä hiilessä kiinni vain kolme hiiltä
- **erikoistapaus: fenolit** ovat yhdisteitä, joissa hydroksyyliryhmä on **suoraan** kiinni bentseenirenkaassa
- nimetään *"-oli"*

### Aldehydit / karbonyyliryhmä (-C=O)
- karbonyyliryhmän hiili **hiiliketjun päässä**
  - kutsutaan myös aldehydiryhmäksi
- voi hapettua karboksyylihapoksi
- voi pelkistyä (primääriseksi) alkoholiksi
- nimetään *"-naali"*

### Ketonit / karbonyyliryhmä (-C=O)
- karbonyyliryhmän hiili **hiiliketjun keskellä**
  - kutsutaan myös ketoryhmäksi
- voi pelkistyä (sekundääriseksi) alkoholiksi
- nimetään *"-oni"*

### Karboksyylihapot (-COOH)
- voi sisältää myös useamman karboksyyliryhmän
- ei voi hapettua
- voi pelkistyä aldehydiksi
- tyypillisiä reaktioita myös protolysoituminen, neutraloituminen ja esteröityminen
- nimetään *-happo*


### Esterit (-COO-)
- nimetään *"-oaatti"*

### Eetterit (-O-)

### Amiinit / aminoryhmä (-NH<sub>2</sub>) HÄH?

### Amidit / amidiryhmä (-CO-NH-) HÄH?

### Alkyylihalidit





## Hiiliyhdisteiden nimeäminen
Tulossa jos tulossa

# Hienoja löytöjä
- Hapon konjugaattiemäs syntyy kun happo luovuttaa protonin - kun emäs vastaanottaa protonin, syntyy sitä vastaava konjugaattihappo.
- protolyysi = happo-emäsreaktio
- neutraloituminenkin on protolyysireaktio
- liukeneminenkin voi olla eksotermista
- miksi hapot syövyttää: https://www.luma.fi/kysymykset/2009/01/16/syovyttavyys/
  - tldr: hapon luovuttama vetyioni on voimakas hapetin epäjaloille metalleille ja pystyy pilkkomaan orgaanisia yhdisteitä
- suolasillan ioneita tarvitaan vain galvaanisen kennon ensimmäisten hetkien aikana: kun reaktio on käynnistynyt, elektrolyyttiin syntyvät ionit alkavat kuljettaa varausta suolasiltaa pitkin
- orbitaalien täyttymisen "puoliksi täysi" -tila
- *kovalenttinen = ko-valenttinen = yhteisulkoelektroninen*

# Asioita jotka *vain pitää muistaa*
- hapettuminen = elektronien luovuttamista
- pelkistyminen = elektronien vastaanottamista
- negatiivinen ioni = anioni
- positiivinen ioni = kationi
- happo = aine joka voi luovuttaa protonin
- emäs = aine joka voi vastaanottaa protonin

# Omituisuuksia (lähinnä muistilista selvitettäväksi myöhemmin)
- Miksi metallisidosta ei luokitella heikoksi sidokseksi, vaikka sen avulla ei muodostu suoloja eikä molekyylejä? Myös kahden eri metallin välillä voi olla metallisidos, eikä silti esimerkiksi pronssille muodosteta molekyylikaavaa CuSn tai messingille CuZn.
- Onko ionisidoksessa välttämättä kaikilla atomeilla oktetti? Samalla atomilla kuitenkin monta eri hapetuslukua?
- Hapolle ja emäkselle on monta määritelmää: "riittääkö" Brønsted-Lowryn teoria?
- Miten puskuriliuokset toimii?
- Miten kompleksit ja koordinaatiosidokset toimii? Mitä elektroneja luovutetaan, onko kovalenttinen sidos?


# Lähteet
- https://fi.wikipedia.org/wiki/Orgaaninen_kemia
- http://www.oppi.uef.fi/wanda/users/tanevala/KPF2/www/sis-luettelo.htm
- https://otavanopisto.muikkuverkko.fi/workspace/ke2-ihmisen-ja-elinympariston-kemiaa/materials#p-143029
- https://www.luma.fi/kysymykset/
- KE1, Päivi Koskinen ja Ari Koskinen, SanomaPro 2016
